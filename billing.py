...
class BillingClass:
    ...
    def __init__(self,root):
        ...
        # ====title====
        self.icon_title = PhotoImage(file=os.path.join(root_dir,"image\log1.png"))
        ...
        self.var_billno.set(str(x))

        CustomerFrame = Frame(self.root, bd=4, relief=RIDGE, bg="white")
        CustomerFrame.place(x=420, y=110, width=530, height=70)

        cTitle = Label(CustomerFrame, text="Customer Detalis", font=("goudy old style", 15), bg="lightgrey").pack(side=TOP, fill=X)
        lbl_name = Label(CustomerFrame, text="Name", font=("goudy new roman", 15), bg="white").place(x=5, y=35)
        txt_name = Entry(CustomerFrame, textvariable=self.var_cname, font=("times new roman", 13),bg="lightyellow").place(x=80, y=35, width=180)

        lbl_contact = Label(CustomerFrame, text="Contact No.", font=("goudy new roman", 15), bg="white").place(x=270, y=35)
        ...
        BTitle = Label(billFrame, text="Customer Bill Area", font=("goudy old style", 20, "bold"), bg="skyblue",fg="white").pack(side=TOP, fill=X)
        ...
        btn_print = Button(billMenuFrame, text='Print', command=self.print_bill, cursor="hand2",font=("goudy old style", 15, "bold"), bg="aqua", fg="white")
        ...
        self.cartTitle = Label(cart_Frame, text="Cart \t Total Product: [0]", font=("goudy old style", 13, "bold"), bg="lightgrey")
        ...
        txt_p_name = Entry(Add_cartWidgetsFrame, textvariable=self.var_pname, font=("times new roman", 15), bg="lightgrey", state='readonly').place(x=5, y=35, width=190, height=22)

        lbl_p_price = Label(Add_cartWidgetsFrame, text="Price Per Qty", font=("times new roman", 15), bg="white").place( x=230, y=5)
        txt_p_price = Entry(Add_cartWidgetsFrame, textvariable=self.var_price, font=("times new roman", 15), bg="lightgrey", state='readonly').place(x=230, y=35, width=150, height=22)

        lbl_p_qty = Label(Add_cartWidgetsFrame, text="Quantity", font=("times new roman", 15), bg="white").place(x=390,y=5)
        txt_p_qty = Entry(Add_cartWidgetsFrame, textvariable=self.var_qty, font=("times new roman", 15), bg="lightgrey").place(x=390, y=35, width=120, height=22)

        self.lbl_instock = Label(Add_cartWidgetsFrame, text="In stock", font=("times new roman", 15), bg="white")
        self.lbl_instock.place(x=5, y=70)

        btn_clear_cart = Button(Add_cartWidgetsFrame, text="Clear", command=self.clear_cart, font=("times new roman", 15, "bold"), bg="grey", cursor="hand2").place(x=180, y=70, width=150,height=30)
        ...
        # =====================footer=====================================
        footer = Label(self.root, text="IMS-Inventory Management System", font=("times new roman", 11), bg="grey", fg="white", bd=0, cursor="hand2").pack(side=BOTTOM, fill=X)
    ...
    def get_input(self, num):
        numb = self.var_cal_input.get() + str(num)
        self.var_cal_input.set(numb)
    ...
    def generate_bill(self):
        if self.var_cname.get() == '':
            ...
            if op > 0:
                fp = open(f'bill/{self.var_billno.get()}.txt', 'w')
                fp.write(self.txt_bill_area.get('1.0', END))
                fp.close()
                messagebox.showinfo("Saved", f"Bill no : {self.var_billno.get()}saved successfully")
                self.chk_print = 1
            else:
                return
...
Bill No:{self.var_billno.get()}\t\t\tDate:{str(time.strftime("%d/%m/%Y"))}
{str("=" * 47)}
