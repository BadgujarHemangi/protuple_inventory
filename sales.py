...
class salesClass:
    def __init__(self,root):
        ...
        self.bill_area=Text(bill_Frame,font=("goudy old style",10,"bold"),bg="lightyellow",yscrollcommand=scrolly2.set)
        scrolly2.pack(side=RIGHT,fill=Y)
        scrolly2.config(command=self.bill_area.yview)
        self.bill_area.pack(fill=BOTH,expand=1)

        #======================Image=================================
        self.bill_photo=Image.open("C:\\Users\\Hema's PC\\vscode\\protuple_inventory\\image\\customer.png")
    ...
    def search(self):
        if self.var_invoice.get()=="":
            messagebox.showerror("Error","Invoice no must br required",parent=self.root)
        else:
