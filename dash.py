...
class IMS:
    ...
    def __init__(self,root):
       ...
       #====Left Menu===
        self.MenuLogo=Image.open(os.path.join(root_dir,"image/menu.png"))
        ...
        btn_exit=Button(LeftMenu,text="Exit",command=self.exit,image=self.icon_side,compound=LEFT,padx=6,anchor="w",font=("times new roman",20,"bold"),bg="white",bd=3,cursor="hand2").pack(side=TOP,fill=X)
    ...
    def exit(self):
        op=messagebox.askyesno("Exit","Do you really want to exist")
        if op>0:
         self.root.destroy()
